...
import { applyClientFirstStructure } from "./formatter/clientFirst";
import { WebflowSiteStructure, WebflowReadyNode } from "./types";
import {
  normalizeCssProperties,
  extractTailwindVarMap,
  resolveComplexSelectors,
...
program
  ...
  .action(async (directory, options) => {
    ...
    const siteStructure: WebflowSiteStructure = {
          ...
          if (styles) {
            for (const [selector, breakpoints] of Object.entries(styles)) {
              ...
              for (const [bp, props] of Object.entries(breakpoints)) {
                if (!siteStructure.globalStyles![selector][bp as keyof typeof breakpoints]) {
                  siteStructure.globalStyles![selector][bp as keyof typeof breakpoints] = {};
                }
                Object.assign(siteStructure.globalStyles![selector][bp as keyof typeof breakpoints]!, props);
              }
            }
          }
    ...
    const twVarMap = extractTailwindVarMap(
      (siteStructure.globalStyles || {}) as any
    );
      ...
      for (const [selector, bpStyles] of Object.entries(siteStructure.globalStyles)) {
        for (const [bp, props] of Object.entries(bpStyles)) {
          if (props && typeof props === "object") {
            (siteStructure.globalStyles[selector] as any)[bp] =
              normalizeCssProperties(props as Record<string, string>, twVarMap);
          }
        }
      }
    }
    ...
    const complexSelectors: Record<string, Record<string, Record<string, string>>> = {};
    if (siteStructure.globalStyles) {
      for (const selector of Object.keys(siteStructure.globalStyles)) {
        ...
        if (baseSelector.includes(" ")) {
          complexSelectors[selector] = siteStructure.globalStyles[selector] as any;
        }
      }
    }
