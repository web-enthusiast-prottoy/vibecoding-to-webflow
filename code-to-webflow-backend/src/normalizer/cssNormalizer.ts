...
// ------------------------------------
function resolveVendorPrefix(key: string): string | null {
	if (
...
// ------------------------------------
function resolveBackground(value: string): { key: string; value: string } {
...
function expandBorderRadius(val: string): Record<string, string> {
	const parts = val.match(/(?:[^\s(]+|\((?:[^()]+|\([^()]*\))*\))+/g);
	if (!parts || val.includes("/")) return { "border-radius": val };
...
function expandBorder(val: string, side?: string): Record<string, string> | null {
	...
	sides.forEach((s) => {
		if (width) res[`border-${s}-width`] = width;
		if (style) res[`border-${s}-style`] = style;
		if (color) res[`border-${s}-color`] = color;
	});
...
export function normalizeHexAlpha(value: string): string {
	return value.replace(
		...
		(match) => {
			...
			if (hex.length === 3 || hex.length === 6) return match;
...
export function normalizeCssProperties(
	...
	for (const [rawKey, rawValue] of Object.entries(properties)) {
		...
		if (resolved === null) continue; // Skip unsupported prefixed prop
		key = resolved;
 ...
 */
export function resolveComplexSelectors(
	complexSelectors: Record<string, Record<string, Record<string, string>>>,
	nodes: WebflowReadyNode[],
): WebflowReadyNode[] {
	// Walk tree with each node's ancestor class list
	function walk(node: WebflowReadyNode, ancestorClasses: string[][]): void {
...
function matches(
	...
	if (parts.length === 2) {
		...
		if (!matchesSingle(node, childPart)) return false;
		// Check ancestors
		if (parentPart.startsWith(".")) {
			const parentClass = parentPart.substring(1);
			return ancestorClasses.some((classList) =>
				classList.includes(parentClass),
			);
		}
...
function matchesSingle(node: WebflowReadyNode, selector: string): boolean {
	...
	if (node.tag?.toLowerCase() === lower) return true;
	if (node.type.toLowerCase() === lower) return true;
	// Common aliases
	if (lower === "a" && node.type === "Link") return true;
	if (lower === "img" && node.type === "Image") return true;
	if (lower === "li" && node.type === "ListItem") return true;
	if ((lower === "ul" || lower === "ol") && node.type === "List") return true;
	return false;
}
 ...
 */
export function extractTailwindVarMap(
	globalStyles: Record<string, Record<string, Record<string, string>>>,
): Record<string, string> {
	...
	for (const [selector, bpStyles] of Object.entries(globalStyles)) {
		...
		const mainProps = (bpStyles as any).main || bpStyles;
		if (!mainProps || typeof mainProps !== "object") continue;

		for (const [prop, val] of Object.entries(
			mainProps as Record<string, string>,
		)) {
			if (prop.startsWith("--")) {
				map[prop] = val;
			}
