...
function processAssetUrl(url: string, baseDir?: string): string {
...
type BreakpointId = keyof BreakpointStyles;

function inferVariableType(
	value: string,
	varMap?: Record<string, string>,
	depth = 0,
): "Color" | "Size" | "FontFamily" | "Number" | "Percentage" {
	...
	if (lowerVal.startsWith("var(") && depth < 3 && varMap) {
		...
		if (varMatch) {
			...
			if (resolved) return inferVariableType(resolved, varMap, depth + 1);
		}
	}
 ...
 */
function getVariableCollection(name: string, type: string): string {
	const n = name.toLowerCase();

	// 1. Colors -> Base Collection
	if (type === "Color") return "Base Collection";
...
function parseValueForVariable(type: string, value: string): any {
...
function isSupportedProperty(prop: string): boolean {
...
export function parseStyleTag(css: string): {
	...
	const styles: Record<string, BreakpointStyles> = {};
	...
	const unsupportedVarsByMode = new Map<string, Record<string, string>>();

	root.walkRules((rule) => {
		// If rule is inside a keyframe, skip it (we already captured the whole at-rule)
		let rp = rule.parent;
		while (rp && rp.type !== "root") {
			if (rp.type === "atrule" && (rp as any).name === "keyframes")
				return;
			rp = rp.parent;
		}

		const selectors = (rule.selectors || [rule.selector]).map((s) =>
			s.replace(/\s+/g, " ").trim(),
		);
		...
		const unsupportedRuleDeclarations: Record<string, string> = {};
		const supportedRuleDeclarations: Record<string, string> = {};

		const varModeMap: Record<BreakpointId, string> = {
			main: "Base Mode",
			medium: "Tablet",
		...
		rule.walkDecls((decl) => {
			const isUniversal = selectors.some(
				(s) => s.startsWith("*") || s === ":root",
			);
			let prop = decl.prop;
...
function convertHtmlNode(node: any, baseDir?: string): WebflowReadyNode | null {
	...
	if (node.nodeName.startsWith("#") || !node.tagName) return null;
	...
	const rawInlineStyles: Record<string, string> = {};
	...
	(node.attrs || []).forEach((attr: any) => {
		if (attr.name === "class") {
			...
			attr.value.split(";").forEach((s: string) => {
				const parts = s.split(":");
				if (parts.length < 2) return;
				const key = parts[0].trim();
				const val = parts.slice(1).join(":").trim();
				if (key && val) rawInlineStyles[key] = val;
			});
		} else {
	...
	if (["h1", "h2", "h3", "h4", "h5", "h6"].includes(tagName))
		...
		if (tagName === "script" || tagName === "style") {
			const innerCode = (node.childNodes || [])
				.map((c: any) => (c.nodeName === "#text" ? c.value : ""))
				.join("");
			...
			textVal = serialize({ childNodes: [node] } as any);
		}
...
export function parseHtml(code: string, baseDir?: string): ParseResult {
	const document = parse(code);
	const globalStyles: Record<string, BreakpointStyles> = {};
	const globalVariables: WebflowVariable[] = [];
	let keyframes = "";
	let unsupportedCss = "";

	const findStyleTags = (node: any) => {
		if (node.tagName === "style") {
			...
			if (css) {
				...
				if (tagKeyframes) keyframes += tagKeyframes + "\n";
				if (tagUnsupportedCss) unsupportedCss += tagUnsupportedCss + "\n";
				if (variables.length > 0) {
					globalVariables.push(...variables);
				}
				for (const [selector, breakpoints] of Object.entries(styles)) {
					if (!globalStyles[selector]) globalStyles[selector] = {};
					for (const [bp, props] of Object.entries(breakpoints)) {
						const typedBp = bp as keyof BreakpointStyles;
	...
	const body = htmlNode
		? (htmlNode as any).childNodes.find((n: any) => n.tagName === "body")
		: null;

	if (!body) return { nodes: [], globalStyles, collections: [] };

	const nodes: WebflowReadyNode[] = [];
	body.childNodes.forEach((child: any) => {
		const converted = convertHtmlNode(child, baseDir);
		if (converted) nodes.push(converted);
	});

	let collections: WebflowCollection[] | undefined = undefined;
	if (globalVariables.length > 0) {
		const groups: Record<string, WebflowVariable[]> = {};

		globalVariables.forEach((v) => {
			const group = (v as any).group || "Base Collection";
