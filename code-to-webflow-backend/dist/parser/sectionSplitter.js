...
function getStructuralFingerprint(node) {
    const getObj = (n) => ({
        t: n.type,
        g: n.tag,
...
function getVariantFingerprint(node) {
    const getObj = (n) => ({
        cls: n.classes,
        c: n.children?.map(getObj) || [],
    });
...
export function splitSiteStructure(siteStructure, outputDir, baseName) {
    ...
    const sections = [];
    let mainClasses = ["main-wrapper"];
    function getUsedClasses(node, classSet) {
        (node.classes || []).forEach((c) => classSet.add(c));
        (node.children || []).forEach((child) => getUsedClasses(child, classSet));
    }
    function processNode(node) {
        ...
        if (classes.length > 0)
            name = classes[0];
        const isWrapper = classes.includes("page-wrapper") ||
    ...
    const selectorToSections = new Map();
    ...
    const sharedStyles = {};
    const sectionSpecificStyles = sections.map(() => ({}));
    allStyleSelectors.forEach((selector) => {
    ...
    sections.forEach((section, idx) => {
        ...
        fs.writeFileSync(path.join(projectDir, fileName), JSON.stringify(sectionData, null, 2));
