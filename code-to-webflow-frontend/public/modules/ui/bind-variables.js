...
function normalizeUnitless(val) {
    const s = val.trim();
    if (/^\d+\.?\d*$/.test(s))
        return s;
    return null;
...
function scoreVariableName(prop, varName) {
...
function findMatch(prop, value, index) {
...
function getOptions(bp) {
    return bp === "main" ? undefined : { breakpoint: bp };
}
function showInlineReview(grouped, onApply, onCancel) {
    const container = document.getElementById("bind-review-container");
    const categoriesEl = document.getElementById("bind-review-categories");
    const detailsEl = document.getElementById("bind-review-details");
    const applyBtn = document.getElementById("bind-review-apply-inline");
    const cancelBtn = document.getElementById("bind-review-cancel-inline");
    if (!container) {
        console.error("[BIND] bind-review-container NOT FOUND");
        return;
    }
    if (!categoriesEl) {
        console.error("[BIND] bind-review-categories NOT FOUND");
        return;
    }
    if (!detailsEl) {
        console.error("[BIND] bind-review-details NOT FOUND");
        return;
    }
    if (!applyBtn) {
        console.error("[BIND] bind-review-apply-inline NOT FOUND");
        return;
    }
    if (!cancelBtn) {
        console.error("[BIND] bind-review-cancel-inline NOT FOUND");
        return;
    }
    const categories = [
    ...
    // Wire buttons
    applyBtn.onclick = () => {
        const selected = { colors: [], typography: [], border: [], spacing: [] };
        for (const cat of categories) {
            const cb = document.getElementById(`bind-cat-${cat.key}`);
            if (cb && cb.checked) {
...
// Discovery Phase
// ============================================================
async function discoverBindings(btnRef) {
    _clearLog();
    complexValueEmbeds.length = 0;
    // Hide any previous review container
    const prevContainer = document.getElementById("bind-review-container");
    if (prevContainer)
    ...
    try {
        ...
        const grouped = { colors: [], typography: [], border: [], spacing: [] };
        let skippedCount = 0;
        for (let sIdx = 0; sIdx < allStyles.length; sIdx++) {
            ...
            for (let bIdx = 0; bIdx < BREAKPOINTS.length; bIdx++) {
                ...
                let properties;
                try {
                    properties = await withTimeout(style.getProperties({ breakpoint: bp }), 8000, `getProperties-${styleName}-${bp}`);
                }
                catch (e) {
        ...
        if (skippedCount > 0)
            log(`${skippedCount} matches skipped due to low semantic score.`);
        return grouped;
    }
    catch (err) {
...
// Apply Phase
// ============================================================
async function applyBindings(grouped, btnRef) {
    var _a, _b;
    _clearLog();
    complexValueEmbeds.length = 0;
    const progressContainer = document.getElementById("progress-container");
    ...
    let boundCount = 0;
    let fallbackCount = 0;
    const byStyleBp = new Map();
    for (const m of allMatches) {
        const key = `${m.styleName}::${m.breakpoint}`;
        if (!byStyleBp.has(key))
            byStyleBp.set(key, []);
    ...
    for (const [, matches] of byStyleBp) {
        ...
        const plainBindings = [];
        const paddingBindings = [];
        const marginBindings = [];
        for (const m of matches) {
            if (PADDING_PROPS.has(m.property))
                paddingBindings.push(m);
            else if (MARGIN_PROPS.has(m.property))
                marginBindings.push(m);
            else
                plainBindings.push(m);
        }
        // Plain bindings
        for (const m of plainBindings) {
            try {
                await withTimeout(style.setProperty(m.property, m.variable, getOptions(bp)), 8000, `bind-${styleName}-${m.property}-${bp}`);
                boundCount++;
        ...
        if (paddingBindings.length > 0) {
            ...
            for (const side of sides) {
                if (paddingMap.has(side)) {
                    ...
                    try {
                        ...
                        fullGroup[side] = (_a = props === null || props === void 0 ? void 0 : props[side]) !== null && _a !== void 0 ? _a : "0px";
                    }
                    catch (_c) {
                        fullGroup[side] = "0px";
                    }
            ...
            try {
                await withTimeout(style.setProperties(fullGroup, getOptions(bp)), 10000, `bind-padding-${styleName}-${bp}`);
        ...
        if (marginBindings.length > 0) {
            ...
            for (const side of sides) {
                if (marginMap.has(side)) {
                    ...
                    try {
                        const props = await withTimeout(style.getProperties({ breakpoint: bp }), 3000, `getProps-${side}`);
                        fullGroup[side] = (_b = props === null || props === void 0 ? void 0 : props[side]) !== null && _b !== void 0 ? _b : "0px";
                    }
                    catch (_d) {
                        fullGroup[side] = "0px";
                    }
            ...
            try {
                await withTimeout(style.setProperties(fullGroup, getOptions(bp)), 10000, `bind-margin-${styleName}-${bp}`);
    ...
    log(`Apply complete. Bound ${boundCount} properties. Fallbacks: ${fallbackCount}.`, boundCount > 0 ? "success" : "warn");
    if (fallbackCount > 0) {
...
// Public Entry
// ============================================================
export async function handleBindVariables(btnRef) {
    ...
    showInlineReview(grouped, async (selected) => {
        try {
