...
import type { WebflowVariable, WebflowCollection, WebflowVariableValue, LogLevel } from "../../types.js";
...
export function injectIncrementProgress(fn: (amount?: number) => void): void {
...
export function resolveValueForCreate(type: string, serVal: any): any {
	...
	if (typeof effectiveValue === "string") {
		...
		if (varMatch) {
			...
			if (cached && cached.cssName) {
				log(
					`    [RESOLVE] Resolved var() alias for ${type}: "${varName}" -> "var(${cached.cssName})"`,
				);
			...
			log(
				`    [RESOLVE] Unresolved var() for ${type}: "${trimmed}" — storing as CustomValue`,
			);
		...
		if (
			...
			log(
				`    [RESOLVE] Complex CSS function for ${type}: "${trimmed}" — wrapping as CustomValue`,
			);
			return { type: "custom", value: trimmed };
		}
		...
		if (
			...
			if (match) {
				...
				log(
					`    [RESOLVE] String to Size object for ${type}: "${trimmed}" -> ${JSON.stringify(res)}`,
				);
				return res;
			}
		}
	...
	if (
		...
		if (
			...
			log(
				`    [RESOLVE] Direct Size object for ${type}: ${JSON.stringify(res)}`,
			);
			return res;
		}
	}
...
export async function pasteCollections(
	...
	const existingCollections = await (webflow as any).getAllVariableCollections();
	const defaultCollection =
		typeof (webflow as any).getDefaultVariableCollection === "function"
			? await (webflow as any).getDefaultVariableCollection()
	...
	for (const [colName, colData] of collectionMap.entries()) {
		...
		if (!targetCol) {
			...
			try {
				targetCol = (await withTimeout(
					(webflow as any).createVariableCollection(colName),
					15000,
		...
		for (const serializedVar of colData.variables) {
			...
			let webflowVar =
				varMap.get(expectedName) || varMap.get(serializedVar.name);
			...
			if (!webflowVar) {
				...
				try {
					...
					log(
						`    [CREATE] Initializing ${serializedVar.type} variable "${name}" with value: ${JSON.stringify(defaultValue)}`,
					);
					switch (serializedVar.type) {
						case "Color":
							webflowVar = (await withTimeout(
								targetCol.createColorVariable(name, defaultValue),
								10000,
								"createColorVariable",
							)) as any;
							break;
						case "Size":
							webflowVar = (await withTimeout(
								targetCol.createSizeVariable(name, defaultValue),
								10000,
								"createSizeVariable",
							)) as any;
							break;
						case "FontFamily":
							webflowVar = (await withTimeout(
								targetCol.createFontFamilyVariable(name, defaultValue),
								10000,
						...
						case "Number":
							webflowVar = (await withTimeout(
								targetCol.createNumberVariable(name, defaultValue),
								10000,
						...
						case "Percentage":
							webflowVar = (await withTimeout(
								targetCol.createPercentageVariable(name, defaultValue),
								10000,
...
export async function syncVariableCacheFromWebflow(): Promise<void> {
	log("Syncing variables from Webflow project...");
	const startTime = Date.now();
	try {
		const collections = (await withTimeout(
			(webflow as any).getAllVariableCollections(),
			10000,
