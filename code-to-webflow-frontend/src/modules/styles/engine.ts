...
import type { NodeStyleMap, CssPropertyMap, LogLevel } from "../../types.js";
...
import { withTimeout, DEFAULT_TIMEOUT, handleStyleError } from "../utils/helpers.js";
...
let _recordUnsupportedCss: ((embed: {
	className: string;
	pseudo: string;
	cssText: string;
}) => void) | null = null;

export function injectEmbedRecorders(
	recordComplex: (cv: {
		className: string;
		property: string;
		value: string;
...
function recordComplexValue(cv: {
	...
	if (_recordComplexValue) _recordComplexValue(cv);
}
...
function recordUnsupportedCss(embed: {
	...
	if (_recordUnsupportedCss) _recordUnsupportedCss(embed);
}
 ...
 */
export function tryResolveVariable(rawName: string): any {
 ...
 */
export function normalizePseudo(rawPseudo?: string): string {
...
export async function applyNodeStyleMap(
	style: any,
...
export async function applyGlobalStyles(
