...
export function showFallbackEmbedsUI(): void {
	...
	if (!(window as any).selectWebflowElement) {
		(window as any).selectWebflowElement = async (index: number) => {
			...
			if (embed && embed.element) {
				try {
					await (webflow as any).setSelectedElement(embed.element);
	...
	(window as any).selectIdElement = async (index: number) => {
		...
		if (embed && embed.element) {
			try {
				...
				log(
					`Failed to select element for ID "${embed.id}": ${e.message}`,
					"warn",
				);
			}
		}
	...
	if (!(window as any).selectComplexValueElement) {
		(window as any).selectComplexValueElement = async (index: number) => {
			...
			if (cv.element) {
				try {
					await (webflow as any).setSelectedElement(cv.element);
					log(
						`Selected element for: .${cv.className} → ${cv.property}`,
						"info",
					);
					return;
				} catch (e: any) {
					log(
						`Failed to select specific element: ${e.message}`,
						"warn",
					);
			...
			log(
				`Searching canvas for an element with class .${cv.className}...`,
				"info",
			);
			...
			try {
						...
						try {
							...
							for (const style of styles) {
								...
								if (name === cv.className) {
									cv.element = el;
									await (webflow as any).setSelectedElement(el);
									log(
										`    [DEBUG] Discovered element for .${cv.className} via project-wide scan`,
										"success",
									);
									return;
								}
				...
				log(
					`Global class style .${cv.className} exists but is unused on canvas.`,
					"info",
				);
			} catch (err: any) {
	...
	if (!(window as any).copyCssAsWebflowJSON) {
		(window as any).copyCssAsWebflowJSON = async (
			index: number,
			btnId: string,
		) => {
			...
			const classTagsHtml =
				embed.classList.length > 0
					? embed.classList
							.map(
								(c) =>
									`<span style="display:inline-block;background:#0f3460;color:#38bdf8;border:1px solid #1e4a8a;border-radius:3px;padding:0 6px;font-size:10px;font-family:monospace;margin-right:3px;">.${c}</span>`,
							)
							.join("")
	...
	if (hasComplex) {
		...
		complexValueEmbeds.forEach((cv, i) => {
			...
			const bpLabel =
				cv.breakpointId && cv.breakpointId !== "main"
