...
export async function handleAuditStyles(findBtnRef: HTMLButtonElement): Promise<void> {
	_clearLog();
	...
	try {
		...
		let foundCount = 0;
		let finished = 0;
		const breakpointsWithIssues = new Set<string>();
		...
		if (foundCount === 0) {
			log(
				"✓ Scan complete: No invalid styles found across all breakpoints.",
				"success",
			);
		} else {
			const bpList = Array.from(breakpointsWithIssues).join(", ");
			log(
				`Scan complete: Found ${foundCount} invalid properties across breakpoints: ${bpList}`,
				"warn",
			);
		}
	} catch (err: any) {
