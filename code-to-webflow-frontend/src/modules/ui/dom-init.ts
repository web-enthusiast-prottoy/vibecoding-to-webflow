...
import type { SitePayload } from "../../types.js";
import {
	setDomRefs,
	setUploadedPayload,
	uploadedPayload,
...
import { countAllNodes } from "../utils/helpers.js";
import { buildSiteFromJson } from "../builder/orchestrator.js";
import {
	updateProgressBar,
	log,
	setLogAccordionExpanded,
	syncLogPanelState,
	clearLog,
} from "./progress.js";
...
export function init(): void {
	...
	try {
		if (typeof webflow !== "undefined") {
			(webflow as any).setExtensionSize("large").catch(() => {
  ...
  `;
	...
	setDomRefs({
		...
		bindBtn: document.getElementById(
			"bind-variables-btn",
	...
	findBtn.addEventListener("click", () =>
		handleAuditStyles(findBtn),
	);
	bindBtn.addEventListener("click", () =>
		handleBindVariables(bindBtn),
	);
	...
	if (debugToggle) {
		const isDebugMode =
			localStorage.getItem("ignite-debug-mode") === "true";
		if (isDebugMode) {
